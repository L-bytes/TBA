...
source(paste(argv[1],'/src/preprocessing.R', sep=""))
source(paste(argv[1],'/src/differential_expression.R', sep=""))
#source('src/add_uniprot_info.R')
source(paste(argv[1],'/src/clustering.R', sep=""))
source(paste(argv[1],'/src/coexpression_analysis.R', sep=""))
source(paste(argv[1],'/src/validation.R', sep=""))
source(paste(argv[1],'/src/similarity.R', sep=""))
...
#  G12 <- subset(mut, grepl("G12", HGVSp_Short)  &  Hugo_Symbol == "KRAS")
#  group.data[substr(G12$Tumor_Sample_Barcode,1,16), "KRAS"] <- "SNV"
#  group.data[!(rownames(group.data) %in% substr(G12$Tumor_Sample_Barcode,1,16)), "KRAS"] <- "WT"
#  rm(G12)
#  B <- subset(mut, grepl("V600", HGVSp_Short)  &  Hugo_Symbol == hit)
#  group.data[substr(B$Tumor_Sample_Barcode,1,16), hit] <- "SNV"
#  group.data[!(rownames(group.data) %in% substr(B$Tumor_Sample_Barcode,1,16)), hit] <- "WT"
#  rm(B)
# rm(mut)
  time2 <- Sys.time()
...
#  png(file=paste0('output/PCA.png'), width=1920, height=1020)
#  print(plotPCA(ddsVST, intgroup=c(hit)))
#  #print(plotPCA(ntd, intgroup=c(hit)))
#  dev.off()
  
  colnames(d.adj) <- groups
  
  png(file=paste0('output/clusterSamplesVST.png'), width=1920, height=1020)
  ...
  load(file=paste0(argv[3], '/Project/3056657/output/1/training/rdata/sampleSelection.RData'))
...
#  dev.off()
  ...
  write.table(intersect(rownames(d.training.summary[d.training.summary$log2FC >= 0.5,]), rownames(d.training.summary[d.training.summary$log2FC < 1,])), file=paste0('output/', n, '/training/output/differential expression/DE_genes_P1.tsv'), row.names=FALSE, col.names=FALSE, sep='\t')
  write.table(rownames(d.training.summary[d.training.summary$log2FC >= 1,]), file=paste0('output/', n, '/training/output/differential expression/DE_genes_P2.tsv'), row.names=FALSE, col.names=FALSE, sep='\t')
  write.table(intersect(rownames(d.training.summary[d.training.summary$log2FC <= -0.5,]), rownames(d.training.summary[d.training.summary$log2FC > -1,])), file=paste0('output/', n, '/training/output/differential expression/DE_genes_N1.tsv'), row.names=FALSE, col.names=FALSE, sep='\t')
  write.table(rownames(d.training.summary[d.training.summary$log2FC <= -1,]), file=paste0('output/', n, '/training/output/differential expression/DE_genes_N2.tsv'), row.names=FALSE, col.names=FALSE, sep='\t')
...
#  dev.off()
  ...
  write.table(names(geneList.training[1:100]), file=paste0('output/', n, '/training/output/GO/DE_genes_P.tsv'), row.names=FALSE, col.names=FALSE, sep='\t')
  write.table(names(tail(geneList.training,100)), file=paste0('output/', n, '/training/output/GO/DE_genes_N.tsv'), row.names=FALSE, col.names=FALSE, sep='\t')
    
  png(file=paste0('output/', n, '/training/figures/GO/ESBP.png'), width=1920, height=1020)
  gseaplot2(gsea.training.BP, geneSetID = 1:10)
  dev.off()
  
  png(file=paste0('output/', n, '/training/figures/GO/ESMF.png'), width=1920, height=1020)
  gseaplot2(gsea.training.MF, geneSetID = 1:10)
  dev.off()
  #######################################
  ...
  pheatmap(d.validation, color=my_palette, cluster_rows=TRUE, show_rownames=FALSE, cluster_cols=TRUE, show_colnames = FALSE, annotation_col=as.data.frame(group), cex.lab = 2, cex.axis = 2, cex.main = 2)
  dev.off()
...
#  dev.off()
  ...
  write.table(intersect(rownames(d.validation.summary[d.validation.summary$log2FC >= 0.5,]), rownames(d.validation.summary[d.validation.summary$log2FC < 1,])), file=paste0('output/', n, '/validation/output/differential expression/DE_genes_P1.tsv'), row.names=FALSE, col.names=FALSE, sep='\t')
  write.table(rownames(d.validation.summary[d.validation.summary$log2FC >= 1,]), file=paste0('output/', n, '/validation/output/differential expression/DE_genes_P2.tsv'), row.names=FALSE, col.names=FALSE, sep='\t')
  write.table(intersect(rownames(d.validation.summary[d.validation.summary$log2FC <= -0.5,]), rownames(d.validation.summary[d.validation.summary$log2FC > -1,])), file=paste0('output/', n, '/validation/output/differential expression/DE_genes_N1.tsv'), row.names=FALSE, col.names=FALSE, sep='\t')
  write.table(rownames(d.validation.summary[d.validation.summary$log2FC <= -1,]), file=paste0('output/', n, '/validation/output/differential expression/DE_genes_N2.tsv'), row.names=FALSE, col.names=FALSE, sep='\t')
  
  ###DESEQ PLOTS
  d.validation.summary <- d.validation.summary %>% 
    mutate(
      Expression = case_when(log2FC >= log(2) & Pvalue <= 0.1 ~ "Up-regulated",
      log2FC <= -log(2) & Pvalue <= 0.1 ~ "Down-regulated",
      TRUE ~ "Unchanged")
  ...
  print(ggplot(d.validation.summary, aes(log2FC, -log(Pvalue,10))) +
   geom_point(aes(color = Expression), size = 2) +
   xlab(expression("log"[2]*"FC")) + 
   ylab(expression("-log"[10]*"Pvalue")) +
   scale_color_manual(values = c("Up-regulated" = "red", "Down-regulated" = "blue", "Unchanged" = "grey")) +
...
#  dev.off()
  ...
  write.table(names(geneList.validation[1:100]), file=paste0('output/', n, '/validation/output/GO/DE_genes_P.tsv'), row.names=FALSE, col.names=FALSE, sep='\t')
  write.table(names(tail(geneList.validation,100)), file=paste0('output/', n, '/validation/output/GO/DE_genes_N.tsv'), row.names=FALSE, col.names=FALSE, sep='\t')

  png(file=paste0('output/', n, '/validation/figures/GO/ESBP.png'), width=1920, height=1020)
  gseaplot2(gsea.validation.BP, geneSetID = 1:10)
  dev.off()
  
  png(file=paste0('output/', n, '/training/figures/GO/ESMF.png'), width=1920, height=1020)
  gseaplot2(gsea.validation.MF, geneSetID = 1:10)
  dev.off()
  #######################################
  ###   WGCNA coexpression analysis   ###
  #######################################
...
DESimilarity3 <- as.matrix(read.table(paste0('output/3/validation/output/similarity/de.tsv'), sep='\t'))
...
end <- data.frame('DE P LFC < 1' = c(DESimilarity1['Positive','LFC < 1'], DESimilarity2['Positive','LFC < 1'], DESimilarity3['Positive','LFC < 1']),
                  'DE P LFC > 1' = c(DESimilarity1['Positive','LFC > 1'], DESimilarity2['Positive','LFC > 1'], DESimilarity3['Positive','LFC > 1']),
                  'DE N LFC < 1' = c(DESimilarity1['Negative','LFC < 1'], DESimilarity2['Negative','LFC < 1'], DESimilarity3['Negative','LFC < 1']),
                  'DE N LFC > 1' = c(DESimilarity1['Negative','LFC > 1'], DESimilarity2['Negative','LFC > 1'], DESimilarity3['Negative','LFC > 1']),
                  ...
                  'Modules' = c(averageSimilarityModules1, averageSimilarityModules2, averageSimilarityModules3),
                  ...
                  'GO BP' = c(resultSimilarity1['BP',1], resultSimilarity2['BP',1], resultSimilarity3['BP',1]),
                  'GO MF' = c(resultSimilarity1['MF',1], resultSimilarity2['MF',1], resultSimilarity3['MF',1]))
...
R1[nrow(R1) + 1,] <- c('DE P LFC < 1', as.numeric(DESimilarity1['Positive','LFC < 1']))
R1[nrow(R1) + 1,] <- c('DE N LFC < 1', as.numeric(DESimilarity1['Negative','LFC < 1']))
R1[nrow(R1) + 1,] <- c('DE P LFC > 1', as.numeric(DESimilarity1['Positive','LFC > 1']))
R1[nrow(R1) + 1,] <- c('DE N LFC > 1', as.numeric(DESimilarity1['Negative','LFC > 1']))
...
png(file='output/1/validation/figures/similarity/violin.png', width=1920, height=1020)
ggplot(R1, aes(x=Type, y=Similarity, fill=Type)) + 
  ...
  scale_x_discrete(limits=c('DE P LFC < 1', 'DE N LFC < 1', 'DE P LFC > 1', 'DE N LFC > 1','GSEA BP', 'GSEA MF', "Module", "Subnetwork", "Significant subnetwork", 'GO genes', 'GO BP', 'GO MF'))
dev.off()

R2 <- data.frame(Type=character(0), Similarity=numeric(0))

R2[nrow(R2) + 1,] <- c('DE P LFC < 1', as.numeric(DESimilarity1['Positive','LFC < 1']))
R2[nrow(R2) + 1,] <- c('DE N LFC < 1', as.numeric(DESimilarity1['Negative','LFC < 1']))
R2[nrow(R2) + 1,] <- c('DE P LFC > 1', as.numeric(DESimilarity1['Positive','LFC > 1']))
R2[nrow(R2) + 1,] <- c('DE N LFC > 1', as.numeric(DESimilarity1['Negative','LFC > 1']))
R2[nrow(R2) + 1,] <- c('GSEA BP', as.numeric(GSEASimilarity2['BP',1]))
R2[nrow(R2) + 1,] <- c('GSEA MF', as.numeric(GSEASimilarity2['MF',1]))
...
R2[nrow(R2) + 1,] <- c('GO genes', as.numeric(resultSimilarity2['genes',1]))
R2[nrow(R2) + 1,] <- c('GO BP', as.numeric(resultSimilarity2['BP',1]))
R2[nrow(R2) + 1,] <- c('GO MF', as.numeric(resultSimilarity2['MF',1]))

R2$Similarity <- as.numeric(R2$Similarity)

png(file='output/2/validation/figures/similarity/violin.png', width=1920, height=1020)
...
R3[nrow(R3) + 1,] <- c('DE P LFC < 1', as.numeric(DESimilarity1['Positive','LFC < 1']))
R3[nrow(R3) + 1,] <- c('DE N LFC < 1', as.numeric(DESimilarity1['Negative','LFC < 1']))
R3[nrow(R3) + 1,] <- c('DE P LFC > 1', as.numeric(DESimilarity1['Positive','LFC > 1']))
R3[nrow(R3) + 1,] <- c('DE N LFC > 1', as.numeric(DESimilarity1['Negative','LFC > 1']))
...
png(file='output/3/validation/figures/similarity/violin.png', width=1920, height=1020)
...
png(file='output/1/validation/figures/similarity/moduleSize.png', width=1920, height=1020)
...
png(file='output/2/validation/figures/similarity/moduleSize.png', width=1920, height=1020)
...
png(file='output/3/validation/figures/similarity/moduleSize.png', width=1920, height=1020)
ggplot(modules3, aes(x=Size1,y=Similarity)) + 
  geom_point(aes(size=Size2, colour = Name1))
dev.off()

print(paste('Similarity for positive DE genes with LFC < 1 across runs:', mean(DESimilarity1['Positive','LFC < 1'], DESimilarity2['Positive','LFC < 1'], DESimilarity3['Positive','LFC < 1'])))
print(paste('Similarity for negative DE genes with LFC < 1 across runs:', mean(DESimilarity1['Negative','LFC < 1'], DESimilarity2['Negative','LFC < 1'], DESimilarity3['Negative','LFC < 1'])))
print(paste('Similarity for positive DE genes with LFC > 1 across runs:', mean(DESimilarity1['Positive','LFC > 1'], DESimilarity2['Positive','LFC > 1'], DESimilarity3['Positive','LFC > 1'])))
print(paste('Similarity for negative DE genes with LFC > 1 across runs:', mean(DESimilarity1['Negative','LFC > 1'], DESimilarity2['Negative','LFC > 1'], DESimilarity3['Negative','LFC > 1'])))
...
#load(file='rdata/differential_expression.RData')
#load(file='rdata/coexpression.RData')
#dir.create(paste0('output/', i, '/training/output/validation'))
#dir.create(paste0('output/', i, '/training/figures/validation'))
#dir.create(paste0('output/', i, '/training/rdata/validation'))
## 
## Load RIMOD data
...
#save(selection.val, file=paste0('output/', i, '/training/rdata/validation/sampleSelectionVal.RData'))
...
#my_palette <- colorRampPalette(c("blue", "white", "red"))(n = 2999)
#
#png(file=paste0('output/', i, '/training/figures/validation/heatmapSNVSubset.png'), width=1920, height=1020)
...
#dev.off()
#
#png(file=paste0('output/', i, '/training/figures/validation/heatmapWTSubset.png'), width=1920, height=1020)
...
#outfolder <- paste0('output/', i, '/training/output/validation/')
#figfolder <- paste0('output/', i, '/training/figures/validation/')
...
#   outfolder <- paste0('output/', i, '/training/output/validation/module_', m)
#   figfolder <- paste0('output/', i, '/training/figures/validation/module_', m)
#   dir.create(outfolder, showWarnings=FALSE)
#   dir.create(figfolder, showWarnings=FALSE)
...
#   outfolder <- paste0('output/', i, '/training/figures/validation/Module_', m, '_hotnet/')
#   dir.create(outfolder, showWarnings=FALSE)
