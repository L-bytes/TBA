...
if (variant != ""){
  ...
  var <- subset(mut, grepl(variant, HGVSp_Short)  &  Hugo_Symbol == hit)
  group.data[substr(var$Tumor_Sample_Barcode,1,16), hit] <- "SNV"
  group.data[!(rownames(group.data) %in% substr(var$Tumor_Sample_Barcode,1,16)), hit] <- "WT"
  rm(var)
  rm(mut)
}
time2 <- Sys.time()
...
for (n in c("1","2","3")){
  ...
  time1 <- Sys.time()
  system(paste('bash src/run_hierarchicalHotnet_modules.sh "', paste(hotnetColors, collapse=' '), '" ', cutOff, n, 'training'))
  time2 <- Sys.time()
  ...
  time1 <- Sys.time()
  system(paste('bash src/run_hierarchicalHotnet_modules.sh "', paste(hotnetColors, collapse=' '), '" ', cutOff, n, 'validation'))
  time2 <- Sys.time()
  print(paste("Hierarchical HotNet:", difftime(time2, time1, units="secs")))
